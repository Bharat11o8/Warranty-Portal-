...
export class DiagnosticController {
    static async checkVendorVerification(req: Request, res: Response) {
        try {
            ...
            const [vvCount]: any = await db.execute('SELECT COUNT(*) as count FROM vendor_verification');

            // Get count of vendor_details records
            const [vdCount]: any = await db.execute('SELECT COUNT(*) as count FROM vendor_details');

            // Get count of vendors in user_roles
            const [vendorCount]: any = await db.execute("SELECT COUNT(*) as count FROM user_roles WHERE role = 'vendor'");
    ...
    static async runMigration(req: Request, res: Response) {
        try {
            ...
            const [columns]: any = await db.execute('SHOW COLUMNS FROM warranty_registrations');
