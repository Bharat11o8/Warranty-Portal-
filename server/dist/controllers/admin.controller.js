...
export class AdminController {
    ...
    static async getAllVendors(_req, res) {
        try {
            const query = `
                SELECT 
                    p.id,
     ...
     */
    static async updateVendorCoordinates(req, res) {
        try {
            const { id } = req.params;
            const { latitude, longitude } = req.body;
            await db.execute('UPDATE vendor_details SET latitude = ?, longitude = ? WHERE user_id = ?', [latitude || null, longitude || null, id]);
            const admin = req.user;
            await ActivityLogService.log({
                adminId: admin.id,
                adminName: admin.name,
    ...
    static async updateVendorProfile(req, res) {
        try {
            ...
            try {
                ...
                const [existingEmail] = await connection.execute('SELECT id FROM profiles WHERE email = ? AND id != ?', [email, id]);
                if (existingEmail.length > 0) {
                    return res.status(400).json({ error: 'Email already in use by another account' });
                }
                await connection.execute('UPDATE profiles SET name = ?, email = ?, phone_number = ? WHERE id = ?', [contact_name, email, phone_number, id]);
                await connection.execute('UPDATE vendor_details SET store_name = ?, store_email = ? WHERE user_id = ?', [store_name, email, id]);
    ...
    static async updateWarrantyStatus(req, res) {
        try {
            ...
            if (warrantyData.customer_email && warrantyData.customer_email.trim()) {
                try {
                    ...
                    if (status === 'validated') {
                        ...
                        await EmailService.sendWarrantyApprovalToCustomer(warrantyData.customer_email, warrantyData.customer_name, warrantyData.uid, warrantyData.product_type, warrantyData.registration_number, warrantyData.car_make, warrantyData.car_model, productDetails, warrantyData.warranty_type, warrantyData.store_name, storeFullAddress, warrantyData.store_email, warrantyData.applicator_name);
                        console.log(`✓ Warranty approval email sent to customer: ${warrantyData.customer_email}`);
                    }
                    else {
                        // Send rejection email to customer
                        await EmailService.sendWarrantyRejectionToCustomer(warrantyData.customer_email, warrantyData.customer_name, warrantyData.uid, warrantyData.product_type, warrantyData.registration_number, rejectionReason, warrantyData.car_make, warrantyData.car_model, productDetails, warrantyData.warranty_type, warrantyData.store_name, storeFullAddress, warrantyData.store_email, warrantyData.applicator_name);
            ...
            try {
                ...
                let vendorName = warrantyData.store_name || null;
                let vendorUserId = null;
                let applicatorName = warrantyData.applicator_name || null;
                ...
                if (vendorEmail && vendorName) {
                    if (status === 'validated') {
                        await EmailService.sendWarrantyApprovalToVendor(vendorEmail, vendorName, warrantyData.customer_name, warrantyData.customer_phone, warrantyData.product_type, warrantyData.registration_number, applicatorName ?? '', warrantyData.uid, warrantyData.car_make, warrantyData.car_model, productDetails, warrantyData.warranty_type);
                        console.log(`✓ Warranty approval email sent to vendor: ${vendorEmail}`);
                    }
                    else {
                        await EmailService.sendWarrantyRejectionToVendor(vendorEmail, vendorName, warrantyData.customer_name, warrantyData.customer_phone, warrantyData.product_type, warrantyData.registration_number, applicatorName ?? '', rejectionReason, warrantyData.uid, warrantyData.car_make, warrantyData.car_model, productDetails, warrantyData.warranty_type);
    ...
    static async getCustomers(_req, res) {
        try {
    ...
    static async deleteCustomer(req, res) {
        try {
            ...
            const [customer] = await db.execute(`SELECT p.id, p.email FROM profiles p 
                 JOIN user_roles ur ON p.id = ur.user_id 
                 WHERE p.email = ? AND ur.role = 'customer' 
                 LIMIT 1`, [email]);
    ...
    static async getAllAdmins(_req, res) {
        try {
            const query = `
                SELECT 
                    p.id,
    ...
    static async createAdmin(req, res) {
        try {
            ...
            await db.execute('INSERT INTO admin_permissions (id, admin_id, is_super_admin, permissions) VALUES (?, ?, 0, ?)', [uuidv4(), userId, JSON.stringify(resolvedPermissions)]);
    ...
    static async updateAdminPermissions(req, res) {
        try {
            ...
            const [target] = await db.execute('SELECT is_super_admin FROM admin_permissions WHERE admin_id = ?', [id]);
            if (target.length === 0) {
            ...
            await db.execute('UPDATE admin_permissions SET permissions = ? WHERE admin_id = ?', [JSON.stringify(normalized), id]);
            // Get admin name for log
            const [adminProfile] = await db.execute('SELECT name FROM profiles WHERE id = ?', [id]);
            await ActivityLogService.log({
                adminId: actor.id,
    ...
    static async deleteAdmin(req, res) {
        try {
            ...
            // Get admin name for log
            const [adminProfile] = await db.execute('SELECT name, email FROM profiles WHERE id = ?', [id]);
    ...
    static async getResubmissions(req, res) {
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 50;
            const offset = (page - 1) * limit;
            const [countResult] = await db.execute('SELECT COUNT(*) as total FROM warranty_resubmissions WHERE status = "pending_review"');
            const totalCount = countResult[0].total;
            const totalPages = Math.ceil(totalCount / limit);
            const [resubmissions] = await db.execute(`
                SELECT 
                    wr.*,
                    p.name as submitted_by_name,
                    p.email as submitted_by_email,
                ...
                LIMIT ? OFFSET ?
            `, [limit, offset]);
            const formatted = resubmissions.map((r) => ({
                ...r,
    ...
    static async approveResubmission(req, res) {
        let connection;
        try {
            const { id } = req.params;
            ...
            if (staging.product_type === 'seat-cover') {
                ...
                await connection.execute('UPDATE pre_generated_uids SET is_used = TRUE, used_at = ? WHERE uid = ?', [usedTimestamp, staging.original_uid]);
            }
            await connection.commit();
    ...
    static async rejectResubmission(req, res) {
        try {
            const { id } = req.params;
            const { notes } = req.body;
            const [rows] = await db.execute('SELECT original_uid FROM warranty_resubmissions WHERE id = ?', [id]);
            if (rows.length === 0)
                return res.status(404).json({ error: 'Resubmission not found' });
            await db.execute('UPDATE warranty_resubmissions SET status = "rejected", admin_notes = ? WHERE id = ?', [notes || 'Rejected by admin', id]);
            const admin = req.user;
            await ActivityLogService.log({
                adminId: admin.id,
                adminName: admin.name,
