...
export const AdminAnalytics = ({ onNavigate }: { onNavigate: (module: AdminModule) => void }) => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [sumRes, timeRes, prodRes, franRes, fraudRes] = await Promise.all([
    ...
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Bottom Row - Franchise Participation & Fraud Detection */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Franchise Leaderboard */}
                <Card className="lg:col-span-2 rounded-[40px] border-orange-50 shadow-[0_15px_40px_rgba(0,0,0,0.02)]">
                    <CardHeader>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-slate-50">
                                        <th className="py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 px-4">Franchise</th>
                                        <th className="py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 px-4 text-center">Warranties</th>
                                        <th className="py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 px-4 text-center text-red-500">Grievances</th>
                                    ...
                                    {franchises?.slice(0, 6).map((f) => (
                                        <tr key={f.id} className="border-b border-slate-50/50 hover:bg-slate-50/50 transition-colors group">
                                            <td className="py-4 px-4">
                                                <p className="font-bold text-slate-700 text-sm truncate max-w-[150px]">{f.store_name}</p>
                                                <p className="text-[10px] text-slate-400">{f.city}</p>
                                            </td>
                                            <td className="py-4 px-4 text-center">
                                                <span className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-600 text-[10px] font-black border border-blue-100">
                                                    {f.warranty_count}
                                                </span>
                                            </td>
                                            ...
                                            <td className="py-4 px-4 text-right font-black text-slate-800 text-sm">
                                                {(f.warranty_count * 10 - f.grievance_count * 5).toFixed(0)}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                ...
                <Card className="rounded-[40px] border-red-50 bg-red-50/10 shadow-[0_15px_40px_rgba(239,68,68,0.03)] overflow-hidden">
                    <CardHeader className="bg-red-50/30 pb-4">
                        <CardTitle className="text-xl font-black text-red-800 uppercase tracking-tight flex items-center gap-2">
                            <ShieldCheck className="h-5 w-5 text-red-600" />
                            Fraud Analysis
                        </CardTitle>
                        <CardDescription className="text-red-600/60">Risk assessment & Trust distribution</CardDescription>
                    </CardHeader>
                    <CardContent className="pt-6 space-y-6">
                        ...
                        <div className="h-[180px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie
                                        data={[
                                            { name: 'High Trust', value: fraudData?.distribution?.high_trust || 0 },
                                            { name: 'Medium Risk', value: fraudData?.distribution?.medium_trust || 0 },
                                            { name: 'High Risk', value: fraudData?.distribution?.low_trust || 0 },
                                        ]}
                                        innerRadius={45}
                                        outerRadius={70}
                                        paddingAngle={5}
                        ...
                        <div className="space-y-3">
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-red-700/60">Top Risk Factors</h4>
                            <div className="space-y-2">
                                {fraudData?.flags?.slice(0, 3).map((flag: any, i: number) => (
                                    <div key={i} className="flex items-center justify-between">
                                        <span className="text-[10px] font-bold text-slate-600 capitalize">{flag.name}</span>
                                        <div className="flex items-center gap-2 flex-1 mx-4">
                                            <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                                <div 
                                                    className="h-full bg-red-500 rounded-full" 
                                                    style={{ width: `${(flag.count / (fraudData?.distribution?.total || 1)) * 100}%` }}
                                                />
                                            </div>
                                        </div>
                                        <span className="text-[10px] font-black text-slate-800">{flag.count}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="pt-4 border-t border-red-100">
                            <div className="bg-red-600 rounded-2xl p-4 text-white shadow-lg shadow-red-200">
                                <p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-80">Estimated Savings</p>
                                <div className="flex items-end gap-1 mt-1">
                                    <span className="text-2xl font-black">₹{fraudData?.impact?.estimated_savings?.toLocaleString()}</span>
                                    <span className="text-[10px] font-bold opacity-80 mb-1">Prevented</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
