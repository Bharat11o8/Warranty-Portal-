...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
    ...
    return (
        <div className="bg-white rounded-[32px] h-full flex flex-col relative overflow-hidden border border-gray-100 hover:shadow-2xl transition-all duration-500 group">
            ...
            {/* Actions Panel — pure catalogue browsing for everyone, including franchises.
                Franchises punch real orders via Order Management, not from here. */}
            <div className="px-6 pb-6 pt-0 w-full mt-auto">
                <div className="flex items-center justify-between gap-4 pt-4 border-t border-slate-100 w-full">
                    ...
                    <Link to={`/product/${product.id}`} className="w-14 h-14 md:w-12 md:h-12 rounded-full bg-brand-orange md:bg-gray-900 text-white flex items-center justify-center transform transition-all duration-300 group-hover:bg-brand-orange group-hover:translate-x-1 group-hover:shadow-lg shadow-md md:shadow-none">
            ...
            {!product.inStock && (
                <div className="mt-2 text-sm text-red-500 font-medium pb-4 text-center">Out of stock</div>
            )}
        </div>
    );
};
