...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
    ...
    return (
        <Link to={`/product/${product.id}`} className="group block h-full">
            <div className="glass-card-premium rounded-2xl overflow-visible relative h-full flex flex-col">
                {/* Swinging NEW Tag */}
                    ...
                    <div className="mt-auto">
                        {hasMultiplePrices ? (
                            <div className="text-brand-orange font-bold space-y-1 text-sm">
                                {(product.price as import('@/types/catalog').ProductPrice).twoRow && (
                                    <p>
                                        2 Row: ₹{(product.price as import('@/types/catalog').ProductPrice).twoRow?.toLocaleString()}{' '}
                                        {showPriceOnward && <span className="text-gray-500 font-normal">onwards</span>}
                                    </p>
                                )}
                                {(product.price as import('@/types/catalog').ProductPrice).threeRow && (
                                    <p>
                                        3 Row: ₹{(product.price as import('@/types/catalog').ProductPrice).threeRow?.toLocaleString()}{' '}
                                        {showPriceOnward && <span className="text-gray-500 font-normal">onwards</span>}
                                    </p>
                                )}
                            </div>
