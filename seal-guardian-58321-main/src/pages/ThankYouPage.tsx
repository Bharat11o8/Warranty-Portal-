...
import { CheckCircle2, Home, LayoutDashboard, MessageSquare, ExternalLink, ShieldCheck } from "lucide-react";
...
const ThankYouPage = () => {
  ...
  const { customerName, productType, registrationId, role, isPublic } = submissionDetails;
        ...
        <Card className="border-none shadow-2xl bg-white/90 backdrop-blur-md overflow-hidden rounded-2xl border border-orange-100/50">
            ...
            {isVendor ? (
              <div className="space-y-6">
                ...
                <div className="flex flex-col items-center gap-2">
                   <div className="bg-slate-900 px-6 py-3 rounded-xl shadow-lg transform hover:scale-105 transition-transform duration-300">
                      <p className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-1">Registration ID</p>
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  <p className="text-xl text-slate-700 leading-relaxed font-medium">
                    Congratulations <span className="text-orange-600 font-bold">{customerName}</span>! Your <span className="font-bold text-slate-800 tracking-tight">{productType === 'seat-cover' ? 'Seat Cover' : 'EV-PPF'}</span> warranty is now active.
                  </p>
                  <div className="bg-slate-900 px-6 py-3 rounded-xl shadow-lg inline-block">
                    <p className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-1">Warranty ID</p>
                ...
                <div className="text-left space-y-5 bg-gradient-to-br from-slate-50 to-orange-50/50 p-6 md:p-8 rounded-2xl border border-orange-100 shadow-sm relative overflow-hidden group">
                  ...
                  <ul className="space-y-4 text-slate-600 relative z-10">
                    <li className="flex gap-4">
                      ...
                      <p className="text-sm">
                        <span className="font-bold text-slate-800 block mb-1 underline decoration-orange-300 decoration-2 underline-offset-4">Access Your Dashboard</span>
                         Login at <span className="font-bold text-orange-600">dashboard.autoformindia.com</span> with your registered email. Check your phone/email for the login OTP.
                      </p>
                    </li>
                    <li className="flex gap-4">
          ...
          <CardFooter className="flex flex-col sm:flex-row gap-4 justify-center pb-12 pt-4 px-6">
            <Button asChild className="bg-slate-900 hover:bg-slate-800 text-white min-w-[200px] h-12 rounded-xl shadow-lg hover:shadow-xl transition-all group">
              <Link to={isVendor ? "/dashboard/vendor" : "/login"}>
